...
\begin{align*}
	...
	v(x, y) &= -2xy
\end{align*}
This basic example shows why each complex valued function can be represented as functions of $2$ \textit{real} variables.
...
\begin{enumerate}
	...
	\textbf{Proof:} \\
	$ \alpha = |z| < 1.$ Clearly each term is less than the next in this sequence and 
	looks to go to $0$ as the limit so we try:
	\begin{align*}
		\left| \alpha_n - 0 \right| < \epsilon
	\end{align*}
	By the Archimedean principle we can choose $m \geq n$ so that  $a_m \leq a_n$ and:
	\begin{align*}
		\left| \alpha_m - 0 \right| \leq \left| \alpha_n - 0 \right| < \epsilon \\
		\therefore \left| \alpha_m\right| < \epsilon
	\end{align*}
	And so the sequence converges to $0$ by definition. \qed
	\\
