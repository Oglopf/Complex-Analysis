...
\begin{enumerate}
	...
	\textbf{Proof:} \\
	Likely just want an absolute convergence where it's shown each successive term is less than the next, and so 
	the whole sequence converges if its absolute value converges. Could also use a Cauchy Sequence though.
	...
	Therefore this sequence is monotonic, it only decreases each new term.

	So then let $\epsilon > 0$ and see by definition:
	\begin{align*}
		|\alpha - 0| < \epsilon^n \\
		|\frac{1}{z} - 0| < \epsilon^n \\
		\left|\frac{1}{z} - 0\right|^{-n} < \epsilon \\
		\frac{1}{|z|^n} < \epsilon \\
	\end{align*} \qed
	\\
